...
const AddSuiteModal = ({ isModalOpen, setModalOpen, suites }: AddSuiteModalProps) => {
  ...
  const handleAddSuite = (e: React.FormEvent) => {
    ...
    addSuite({
      ...
      if (res.length == 36) { // TODO: Пофиксить когда будет нормальный респонс
  ...
  const findParentSuiteName = () => {
    const parentSuiteName = suites.suites.find((suite) => suite.suiteId === parentSuite)?.suiteName
    if (parentSuiteName) {
      ...
      <div className="add-suite">
        ...
        <form className="add-suite__form" onSubmit={handleAddSuite}>
          <div className="add-suite__wrapper">
            ...
            <Input {...nameForm} id="name" name="name" required={true} className="add-suite__input" type="text"
                   placeholder="For example: Web application" />
            <div className="add-suite__error-message">{errorMessage}</div>
          </div>

          <div className="add-suite__wrapper">
          <label htmlFor="parent-suite" className="add-suite__label">Parent Suite</label>
            <div
              ...
              >
                ...
                {suites.suites.map((suite) => (
                  <li
                    ...
                    role="option"
                    tabIndex={-1}
                    aria-selected={parentSuite === suite.suiteId ? "true" : "false"}
                  >
                    {suite.suiteName}
                  </li>
                ))}
          ...
          <div className="add-suite__buttons">
            <Button onClick={closeAddSuiteModal} style={{ backgroundColor: "var(--neutrals400)" }}>
              Cancel
            </Button>
            <Button type="submit">
              Create
